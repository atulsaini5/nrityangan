...
const socialMedia = [
  { name: 'Facebook', url: 'https://www.facebook.com/wix', icon: (
  ...
  { name: 'Instagram', url: 'https://www.instagram.com/wix/', icon: (
  ...
  { name: 'YouTube', url: 'https://www.youtube.com/user/Wix', icon: (
...
const SocialLinks: React.FC<SocialLinksProps> = ({ isVertical }) => {
  const containerClasses = isVertical
    ? 'hidden lg:flex flex-col space-y-4 fixed top-1/2 -translate-y-1/2 left-4 z-40'
    : 'flex space-x-4 justify-center';

  return (
    <div className={containerClasses}>
      {socialMedia.map((social) => (
        <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" aria-label={social.name} className="text-brand-brown hover:text-brand-gold transition-colors duration-300">
          {social.icon}
        </a>
      ))}
    </div>
